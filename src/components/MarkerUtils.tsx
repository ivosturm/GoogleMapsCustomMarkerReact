...
export function addMarkerDragEvent(marker : google.maps.Marker, latAttrUpdate? : EditableValue<BigJs.Big | string>, lngAttrUpdate? : EditableValue<BigJs.Big | string>, formattedAddressAttr? :  EditableValue<string>){
    google.maps.event.addListener(marker, 'dragend', ( () => {
        
        const lat = latAttrUpdate?.value,
        lng = lngAttrUpdate?.value;
...
export function updateAttribute(coordinate : number, type : string, attrUpdate? : EditableValue<BigJs.Big | string>){
...
export function pinSymbol (color:string,symbol:string,size:string) {
    ...
    let googleSymbol : google.maps.Symbol = {   
        path: pathSymbol,
        fillColor: color,
        fillOpacity: 1,
        strokeColor: '#FFF',
...
export function pinSymbolPath (symbol:string) : string {
	// define pathsymbol to be by default a marker	
    let pathSymbol : string = 'M 0,0 C -2,-20 -10,-22 -10,-30 A 10,10 0 1,1 10,-30 C 10,-22 2,-20 0,0 z M -2,-30 a 2,2 0 1,1 4,0 2,2 0 1,1 -4,0';
    switch(symbol) {
        case 'CIRCLE' :
